...
# Create the certificate using a wildcard for all the domains created in bigdrey.tk
resource "aws_acm_certificate" "bigdrey" {
  domain_name       = "*.bigdrey.tk"
  validation_method = "DNS"
}

# calling the hosted zone
data "aws_route53_zone" "bigdrey" {
  name         = "bigdrey.tk"
  private_zone = false
}

# selecting validation method
resource "aws_route53_record" "bigdrey" {
  for_each = {
    for dvo in aws_acm_certificate.bigdrey.domain_validation_options : dvo.domain_name => {
  ...
  zone_id         = data.aws_route53_zone.bigdrey.zone_id
}

# validate the certificate through DNS method
resource "aws_acm_certificate_validation" "bigdrey" {
  certificate_arn         = aws_acm_certificate.bigdrey.arn
...
resource "aws_lb_listener" "nginx-listner" {
  ...
  port              = 443
  protocol          = "HTTPS"
  certificate_arn   = aws_acm_certificate_validation.bigdrey.certificate_arn
  ...
  "Statement": [
    {
      ...
      "Effect": "Allow",
      "Principal": { "AWS": "arn:aws:iam::${var.account_no}:user/abraham" },
      "Action": "kms:*",
      "Resource": "*"
    }
  ]
